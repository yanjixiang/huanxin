//
//  ForgrtPWViewController.m
//  ZhuoTopSearchrReplacement
//
//  Created by 闫继祥 on 2019/5/30.
...
@interface ForgrtPWViewController ()<UITextFieldDelegate,GCDCountDownDelegate>{
    NSString *user;
    NSString *pw;
    NSString *pwAgain;
    NSString *yzmStr;
...
@implementation ForgrtPWViewController
...
- (void)viewDidLoad {
    ...
    [buttonGetYZM addTarget:self action:@selector(buttonGetYZMClick) forControlEvents:UIControlEventTouchUpInside];
    ...
    UIImageView *imgView2 = [[UIImageView alloc] initWithFrame:CGRectMake(20, line2.height+line2.y+25, 20, 20)];
    imgView2.image = [UIImage imageNamed:@"确认密码"];
    [self.view addSubview:imgView2];
    
    userPW = [[UITextField alloc] initWithFrame:CGRectMake(60, line2.height+line2.y+15, self.view.frame.size.width-80, 40)];
    userPW.delegate = self;
    userPW.placeholder = @"请输入新密码";
...
//    userPWAgain.keyboardType = UIKeyboardTypeNumberPad;
    ...
    [buttonLogin setTitle:@"确认" forState:(UIControlStateNormal)];
