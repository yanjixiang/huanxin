...
- (void)viewDidLoad {
    ...
    [buttonLoginPw setTitle:@"验证码登录" forState:(UIControlStateNormal)];
    ...
    forgetPw.frame = CGRectMake(SCREEN_WIDTH-100, buttonLogin.height+buttonLogin.y+20, 80, 30);
    forgetPw.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:forgetPw];
    [forgetPw addTarget:self action:@selector(forgetPwClick) forControlEvents:UIControlEventTouchUpInside];
    [forgetPw setTitle:@"忘记密码" forState:(UIControlStateNormal)];
    forgetPw.titleLabel.font = [UIFont systemFontOfSize:16];
    [forgetPw setTitleColor:yLightGrayColor forState:(UIControlStateNormal)];
...
//忘记密码
- (void)forgetPwClick {
    ForgrtPWViewController *detail = [[ForgrtPWViewController alloc] init];
